  ...
  render() {
    ...
    return (
      <Map center={userPosition} zoom={this.props.zoom}>
        ...
        <Circle center={userPosition} radius={this.props.filterRadius * 1000}></Circle>
  ...
  renderLocationMarker(location) {
    ...
    return (
      <Marker position={[location.lat, location.long]} icon={LocationsMap.greenIcon}>
        <Popup>{location.name}  {location.distanceToLocationMeters(this.props.lat, this.props.lng)}
        </Popup>
      </Marker>
    )
  }
